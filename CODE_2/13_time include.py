...
def extract_date(soup, source):
    date_tag = None
    if source == 'webmd':
        date_tag = soup.select_one('div.review-date, span.article-date')
    elif source == 'healthboards':
        date_tag = soup.select_one('time, span.date, div.post-date')
    if date_tag:
        try:
...
class UnifiedCrawler:
    ...
    def crawl_webmd_search(self, query, max_pages=200, delay=(1.0,2.0), start_page=1):
        ...
        for page in tqdm(range(start_page, max_pages+1), desc="WebMD pages"):
            ...
            if not r:
                ...
                continue
            soup = BeautifulSoup(r.text,'html.parser')
            links = set(a['href'] for a in soup.find_all('a', href=True) if 'webmd.com' in a['href'] or a['href'].startswith('/'))
    ...
    def crawl_boahealthrds_forum(self, forum_base_url, max_pages=500, start_page=1, delay=(1.0,2.0)):
        for p in tqdm(range(start_page, max_pages+1), desc="HealthBoards pages"):
            ...
            if not r:
                ...
                continue
            soup = BeautifulSoup(r.text,'html.parser')
            thread_links = set(a['href'] for a in soup.find_all('a', href=True) if '/threads/' in a['href'] or '/topic/' in a['href'])
            for tlink in tqdm(thread_links, desc=f"HB threads (page {p})", leave=False):
                self.process_page('healthboards', urljoin(page_url, tlink))
...
def main():
    ...
    parser.add_argument('--out', type=str, default='covid_vaccine_reviews_3.csv')
