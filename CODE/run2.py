...
def train_and_evaluate(csv_path, num_labels, output_pred_path):
    print(f"\n===== {csv_path} 모델 학습 시작 (클래스 수: {num_labels}) =====")
    ...
    # 모델
    # === 오류 해결: num_labels 인자를 명시적으로 전달 ===
    model = ElectraForSequenceClassification.from_pretrained(
        "monologg/koelectra-base-v3-discriminator",
        num_labels=num_labels, # 이 부분이 핵심입니다.
...
binary_acc = train_and_evaluate(
    ...
    num_labels=2,
    output_pred_path="predicted_binary_3.csv" # 파일명 변경 (이전 실패와 구분)
)
...
three_acc = train_and_evaluate(
    ...
    num_labels=3,
    output_pred_path="predicted_three_3.csv" # 파일명 변경 (이전 실패와 구분)
)
