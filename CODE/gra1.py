...
sns.lineplot(data=sentiment_trend, x=sentiment_trend.index, y='Negative_Ratio', marker='o', color='#E34A33')

# 그래프 제목과 축 라벨 설정
plt.title('월별 부정 감정 비율 변화 추이 (2020년 ~ 2023년)', fontsize=16, pad=15)
...
# 주요 이벤트 시점 마커 추가 (예시)
# plt.axvline(pd.to_datetime('2021-01-01'), color='blue', linestyle='--', linewidth=1, alpha=0.7)
# plt.text(pd.to_datetime('2021-01-01'), 95, '백신 초기 접종 시작', rotation=90, color='blue')

plt.tight_layout()
plt.savefig("monthly_negative_ratio_trend_final.png")
plt.close()

print("✅ 시각화 파일 저장 완료: 'monthly_negative_ratio_trend_final.png'")